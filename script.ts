...
function renderList(filtered: Worker[]) {
  ...
  document.querySelectorAll(".btn-detail").forEach(btn => {
    btn.addEventListener("click", (ev) => {
      const id = parseInt((ev.currentTarget as HTMLElement).getAttribute("data-id") || "0", 10);
...
function applyFilters() {
  ...
  const search = (document.getElementById("searchInput") as HTMLInputElement).value.toLowerCase().trim();
